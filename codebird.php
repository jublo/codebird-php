...
class Codebird
{
    public static $_version = "1.2.3004.0027";
    ...
    public static function SHA1($a)
    {
        return base64_encode(hash_hmac(
            ...
            $a,
            Codebird::$key['s'] . '&' 
                . (isset(Codebird::$token['s']) ? Codebird::$token['s'] : ''),
            true));
    }
    ...
    {
        $c = array(
            'consumer_key' => Codebird::$key['k'],
            'version' => '1.0',
            'timestamp' => time(),
            'nonce' => Codebird::Nonce(6),
            'signature_method' => 'HMAC-SHA1'
        );
        $d = array();
        foreach ($c as $e => $_) {
            $d['oauth_' . $e] = Codebird::Url($_);
        }
        if (isset(Codebird::$token['k'])) {
            $d['oauth_token'] = Codebird::Url(Codebird::$token['k']);
        }
        foreach ($f as $e => $_) {
            $d[$e] = Codebird::Url($_);
        }
        ...
        $f = Codebird::SHA1(
            $a . '&'
            . Codebird::Url($b) . '&'
            . Codebird::Url(substr($c, 0, -1)));
        return (($a == 'GET' ? $b . '?' : '') . $c . 'oauth_signature=' . Codebird::Url($f));
    }
    
    public static function CallApi($x, $a, $b, $c)
    {
        if ($c && !isset(Codebird::$token['k'])) {
            return array(
                401,
                array()
            );
        }
        $a = substr($a, 0, 6) == 'oauth/'
            ? 'https://api.twitter.com/' . $a
        ...
        if (Codebird::$ReturnFormat == Codebird::$ReturnFormats['Object']) {
            $g = Codebird::ParseApiReply($g);
        }
        return array(
            $f,
            $g
    ...
    {
        return Codebird::CallApiGet('account/verify_credentials', array(
    ...
    {
        return 'https://api.twitter.com/oauth/authorize?oauth_token=' . Codebird::Url(Codebird::$token['k']);
    }
    ...
    public static function SavedSearches_Destroy($a)
    {
        ...
        return Codebird::CallApiPost('saved_searches/destroy/' . $a['id'], array(), true);
    }
    ...
    public static function Statuses_RetweetedBy_Ids($a)
    {
        ...
        return Codebird::CallApiGet('statuses/' . $a['id'] . '/retweeted_by/ids', $a, true);
    }
    ...
    {
        if (Codebird::$Statuses_PublicTimelineCache['timestamp']
            && Codebird::$Statuses_PublicTimelineCache['timestamp'] + 60 > time()) {
            return array(
                200,
                Codebird::$Statuses_PublicTimelineCache['data']
            );
        }
        $arrReply = Codebird::CallApiGet('statuses/public_timeline', array(), true);
        if ($arrReply[0] == 200) {
            Codebird::$Statuses_PublicTimelineCache = array(
                'timestamp' => time(),
                'data' => $arrReply[1]
            );
        }
    ...
    public static function User_ListId_CreateAll($a)
    {
        ...
        return Codebird::CallApiPost($a['user'] . '/' . $a['list_id'] . '/create_all', $a, true);
    }
    ...
    public static function User_ListId_Members($a)
    {
        ...
        return Codebird::CallApiGet($a['user'] . '/' . $a['list_id'] . '/members/' . $a['id'], $a, true);
    }
    ...
    public static function User_ListId_Members_Delete($a)
    {
        ...
        $a['_method'] = 'DELETE';
        return Codebird::CallApiPost($a['user'] . '/' . $a['list_id'] . '/members', $a, true);
    }
    ...
    public static function User_ListId_Members_Get($a)
    {
        ...
        return Codebird::CallApiGet($a['user'] . '/' . $a['list_id'] . '/members', $a, true);
    }
    ...
    public static function User_ListId_Subscribers($a)
    {
        ...
        return Codebird::CallApiGet($a['user'] . '/' . $a['list_id'] . '/subscribers/' . $a['id'], $a, true);
    }
    ...
    public static function User_ListId_Subscribers_Delete($a)
    {
        ...
        $a['_method'] = 'DELETE';
        return Codebird::CallApiPost($a['user'] . '/' . $a['list_id'] . '/subscribers', $a, true);
    }
    ...
    public static function User_ListId_Subscribers_Get($a)
    {
        ...
        return Codebird::CallApiGet($a['user'] . '/' . $a['list_id'] . '/subscribers', $a, true);
    }
    ...
    public static function User_Lists_Id_Statuses($a)
    {
        ...
        return Codebird::CallApiGet($a['user'] . '/lists/' . $a['id'] . '/statuses', $a, true);
    }
