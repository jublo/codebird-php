...
class Codebird
{
    ...
    {
        if ($this->_statuses_public_timeline_cache['timestamp']
            && $this->_statuses_public_timeline_cache['timestamp'] + 60 > time()) {
            return $this->_statuses_public_timeline_cache['data'];
        }
        $reply = $this->_callApi('GET', 'statuses/public_timeline');
    ...
    {
        return $this->_callApi('GET', 'account/verify_credentials', array(
    ...
    private function _sha1($data)
    {
        ...
        return base64_encode(hash_hmac(
            ...
            $this->_oauth_consumer_secret . '&'
                . ($this->_oauth_token_secret != null ? $this->_oauth_token_secret : ''),
            true
        ));
    }
    ...
    private function _sign($httpmethod, $method, $params = array(), $multipart = false)
    {
        ...
        $signature = $this->_sha1(
            $httpmethod . '&'
            . $this->_url($method) . '&'
            . $this->_url($sign_base_string));
        if ($multipart) {
            $params = array_merge($sign_base_params, array('oauth_signature' => $signature));
            ksort($params);
        ...
        return ($httpmethod == 'GET' ? $method . '?' : '')
            . $sign_base_string
            . '&oauth_signature=' . $this->_url($signature);
    }

    /**
     * Detect HTTP method to use for API call
     */
    private function _detectMethod($method)
    {
        ...
        $httpmethods['POST'] = array(
            // Timeline
            'statuses/destroy/:id', 'statuses/retweet/:id', 'statuses/update',
            // Direct Messages
            'direct_messages/new',
            // Friends & Followers
    ...
    /**
     * Detect if API call should be signed
     */
    ...
    /**
     * Detect if API call should use multipart
     */
    ...
    private function _callApi($httpmethod, $method, $params = array(), $sign = true, $multipart = false)
    {
        ...
        if (isset($authorization)) {
            curl_setopt($ch, CURLOPT_HTTPHEADER, array($authorization, 'Expect:'));
        }
        $reply = curl_exec($ch);
