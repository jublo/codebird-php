...
class Codebird
{
     ...
     * @return void
     */
    public function setProxy($host, $port, $type = "HTTP")
    {
        $this->_proxy['host'] = $host;
        $this->_proxy['port'] = $port;
        $this->_proxy['type'] = strtoupper($type);
    }
    ...
    protected function getCurlInitialization($url)
    {
        ...
        if ($this->hasProxy()) {
            curl_setopt($ch, CURLOPT_PROXYTYPE, constant("CURLPROXY_" . $this->getProxyType()));
     ...
     * @return string The proxy host
     */
    protected function getProxyType()
    {
        return $this->getProxyData('type');
    }
